...
class StreamVGGT(nn.Module, PyTorchModelHubMixin):
    ...
    def inference_aggregator(self, frames, past_key_values=None):     
        past_key_values = [None] * self.aggregator.depth
        ...
        for i, frame in enumerate(frames):
            ...
            for pos in range(24):
                all_aggregated_tokens[pos].append(aggregated_tokens[pos])
    ...
    def inference_long_video(self, images_path, past_key_values=None, save_dir=None, keys=None, mode="crop", max_num=128):  
        ...
        for i, image_path in enumerate(tqdm(images_path, desc="Processing images")):
            images, _, _ = load_and_preprocess_images([image_path], mode=mode)
            ...
            streamvggt_token = {
                'aggregated_tokens_list': [t.cpu().numpy() for t in aggregated_tokens],
                'patch_start_idx': patch_start_idx
            }

            save_path = os.path.join(save_dir, f'{keys[i]}.npy')
            if not os.path.exists(save_path):
                np.save(save_path, streamvggt_token)
            
            if past_key_values[0][0].shape[2] > max_num:
                for i in range(self.aggregator.depth):
